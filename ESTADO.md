...
- **FALTA (con OK del dueño, en orden):** Paso 5 cutover = agregar `oniyouth.xyz`+`www` en Vercel Settings→Domains + DNS en Hostinger (A `@`→76.76.21.21, CNAME `www`→cname.vercel-dns.com, quitar A de Pages); Paso 6 = compra real chica en `oniyouth.xyz` + verificar webhook prod; Paso 7 = merge dev→main (DESPUÉS del DNS) + desactivar GitHub Pages + Deployment Protection (prod pública / preview protegida). Rotar credenciales de prueba cuando se pueda.
