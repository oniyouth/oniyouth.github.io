...
- **Pantalla propia de pago rechazado — YA EN PRODUCCIÓN:** `crear-preferencia.js` `back_urls.failure` → `/?pago=rechazado&codigo=`. `index.html` muestra overlay con estética de la tienda (eyebrow rojo, título Syne, botón blanco "Reintentar con otro medio" → `openCart()`, link WhatsApp, "seguir viendo"). Carrito preservado (localStorage). **PENDIENTE:** el **número de WhatsApp** (constante `WHATSAPP_NUMERO='51XXXXXXXXX'` en index.html; el link se **oculta** mientras no sea válido). Cuando el dueño lo dé → setearlo + `vercel deploy --prod`.
