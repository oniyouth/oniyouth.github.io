...
- **Pantalla propia de pago rechazado (en `dev`/preview, NO en prod aún):** `crear-preferencia.js` `back_urls.failure` → `/?pago=rechazado&codigo=`. `index.html` muestra overlay con estética de la tienda (eyebrow rojo, título Syne, botón blanco "Reintentar con otro medio" → `openCart()`, link WhatsApp, "seguir viendo"). Carrito preservado (localStorage, no se limpia en pago online). **PENDIENTE:** falta el **número de WhatsApp** (constante `WHATSAPP_NUMERO='51XXXXXXXXX'` en index.html; el link se oculta si no es válido). Cuando el dueño lo dé → setearlo + `vercel deploy --prod` para llevarlo a producción.
