...
## Fase 16 — Colores (color × talla) + Guía de tallas (IMPLEMENTADA · requiere migración 008)
Dos features de la ficha de producto. **Cambio de modelo:** una variante pasa de `(producto, talla)` a `(producto, color, talla)`, con **stock independiente por combinación** y **galería propia por color**. Todo el sistema ya se apoyaba en `variante_id` (UUID), así que el cambio es aditivo y **NO toca** `crear-preferencia.js`, `webhook-mp.js` ni las RPC de stock.
- **Migración `008_colores_y_guia_tallas.sql`** (idempotente, NO destructiva): tabla `colores(producto_id, nombre, hex, imagenes[], orden)`; `variantes.color_id` con **backfill** (1 color `'Único'` por producto que hereda `productos.imagenes`; `UPDATE` en su lugar → UUIDs y stock intactos, **pedido real `ONI-68BAF3DF` intacto**); swap unique `(producto_id,talla)`→`(color_id,talla)`; `color_id NOT NULL`; RLS lectura de colores. Tabla `guias_talla(producto_id?, categoria?, imagen_url)` (producto manda sobre categoría), RLS lectura pública.
- **`api/admin.js`:** rutas nuevas `colores` (GET/POST/PATCH/DELETE) y `guias` (GET/POST-upsert/DELETE); alta de producto crea 1 color por defecto ANTES de las variantes; `variantes POST` exige `color_id`; GET de productos **tolerante** (cae a legacy si falta la tabla `colores`).
- **`index.html`:** `loadCatalog` agrupa variantes por color (con **fallback legacy** si `colores` no existe); swatches de color (miniatura = 1ª foto) sobre el selector de talla; al cambiar color cambian galería y tallas (stock de ESE color; sin stock → agotada); enlace **Guía de tallas** → modal con la imagen (resuelve producto→categoría). Portada del grid fija (campo `cover`) para que cambiar de color no la altere. El carrito muestra el color junto a la talla.
- **`admin.html`:** botón **Colores** por producto (crear/renombrar/subir fotos/borrar color), pestaña **Guías** (subir imagen + asociar a producto o categoría + borrar), y la pestaña **Stock** ahora agrupa por color.
- **DESPLIEGUE:** la migración es backward-compatible; el front/panel degradan a legacy si no está. **Migración `008` APLICADA por el dueño (2026-08-20)** — verificado vía REST: `colores` (2 filas `Unico`, 1 por producto, heredaron imágenes), `guias_talla` (existe, vacía), `variantes.color_id` **10/10 poblado (0 nulos)**, stock preservado (venta real `ONI-68BAF3DF` → producto stars **XS=9** intacto). **Desplegado a PREVIEW** (`vercel deploy` → `dpl_DoF9sdzR3SPCGbA7w8yWzUyK4WVv`, alias `oniyouth-dev.vercel.app` re-apuntado). Previews con SSO (302), el dueño los ve logueado en Vercel.
- **PENDIENTE:** que el dueño pruebe en preview → promover a prod (`vercel deploy --prod` + re-apuntar alias `oniyouth-site.vercel.app` / verificar `oniyouth.xyz`); el dueño carga colores reales + stock y sube guías; review visual en móvil. Los swatches solo aparecen con ≥2 colores; el enlace de guía solo si hay guía cargada.

## Fase 17 — Portada editable desde el panel (IMPLEMENTADA · requiere migración 009)
Las imágenes del **hero** (slideshow de la página principal) dejan de estar fijas en `index.html`: el dueño las cambia desde el panel, sin tocar código ni redeployar. También responsive en cualquier dispositivo.
- **Migración `009_configuracion_portada.sql`** (idempotente): tabla genérica clave/valor `configuracion(clave PK, valor jsonb, actualizado_en)`; semilla `hero_slides` = array con las 4 slides WebP actuales (`on conflict do nothing`, no pisa lo guardado); RLS **lectura pública** (anon/authenticated), escritura solo service_role (sin policy de insert/update → bloqueada al navegador).
- **`api/admin.js`:** ruta nueva `config` — `GET ?r=config&clave=hero_slides` (tolerante: si la tabla no existe aún devuelve `valor:null`, no error) y `POST {clave,valor}` (upsert con `Prefer: resolution=merge-duplicates`, service_role). Allowlist de claves + saneo por clave (`hero_slides` = array de URLs, ≤12, ≤2048 chars c/u).
- **`admin.html`:** pestaña nueva **Portada** — grid responsive de miniaturas 16:9 (breakpoints 560/380 px, botones ≥40px táctiles), subir (reusa `uploadImagen`→`/api?r=subir-imagen`, service_role, ≤4 MB), reordenar (↑/↓), quitar (×) y **Guardar cambios** (`POST config`).
- **`index.html`:** slideshow refactorizado a `startHeroSlideshow()` **reiniciable**; `oniLoadHero()` lee `configuracion?clave=eq.hero_slides` con la anon key (best-effort) y reconstruye `#heroSlideshow` en caliente reutilizando las clases `.hero-slide` (sigue siendo full-viewport `object-fit:cover` → **responsive en todo dispositivo**). Si falla o la config no existe, quedan las slides estáticas del HTML. Las URLs relativas de la semilla resuelven contra el origen (mismos archivos de antes → sin flash).
- **VERIFICADO:** `node --check api/admin.js` OK; scripts inline de `index.html` y `admin.html` sin errores de sintaxis (`new Function`).
- **PENDIENTE (manual, requiere Supabase):** aplicar `009_configuracion_portada.sql` en el SQL Editor (sin ella, la pestaña Portada muestra estado vacío y el guardado falla; el hero sigue con las slides estáticas). Luego: desplegar (`vercel deploy` preview / `--prod`), el dueño sube sus fotos y review visual en móvil. Nota: el hero dinámico usa `<img>` WebP directo (sin `<picture>`/fallback), soportado por navegadores modernos.
...
- **Paso 6 (pago real aprobado) HECHO + VERIFICADO (2026-08-18):** `ONI-68BAF3DF` (payment 174514519328) → **firma ok:true (secreto prod), estado `pagado`, stock descontado (oniyouth stars/XS 10→9), correo al cliente Y aviso al admin ENVIADOS por Resend** (MAIL OK, no fallo → dominio `oniyouth.xyz` verificado en Resend, mails desde `pedidos@oniyouth.xyz`). El pipeline de pago productivo funciona end-to-end con dinero real. (Cliente real usó email `gustavoaraujo200534@gmail.com`.)
- **Paso 7 HECHO (2026-08-18):** (1) **merge `dev → main`** — `main` idéntico a `dev` (merge commit; `main` = fuente de verdad de prod). (2) **GitHub Pages**: no se puede desactivar del todo en repo `.github.io` (API 422 "not allowed"); se **liberó el dominio** quitando el archivo `CNAME` → Pages reporta `cname: None`, `oniyouth.xyz` 100% en Vercel. (3) **Deployment Protection Standard** (`ssoProtection.deploymentType=prod_deployment_urls_and_all_previews`): producción pública (dominio + alias 200), **previews protegidos** (oniyouth-dev.vercel.app → 302 a vercel SSO). Verificado.
- **Menores pendientes (opcionales):** rotar credenciales de prueba viejas; cancelar QA `ONI-D595550A` (pendiente; ONI-68BAF3DF es VENTA REAL, no tocar — descontó 1 de stars/XS); redirect canónico www→apex. **Nota flujo Git:** de acá en más, trabajar en `dev` y mergear a `main` para producción; el deploy prod se hace con `vercel deploy --prod` (repo aún NO conectado a Vercel Git; se puede conectar para auto-deploy de main).
